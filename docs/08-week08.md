...
It took 20.2 s to run. This low performance is because the algorithm is visiting every cell and comparing one-by-on with a rotating value for the correct answer from the vector of correct answers. Each object is required to be handled separately in RAM as the process continues.
...
The `adply()` version takes far less coding, but still took 19.5 s to run.
...
The `pmap()` method took 13.582 seconds to run, not much better that the other methods.
...
<hr>
Rendered at <tt>2022-03-12 09:40:27</tt>
