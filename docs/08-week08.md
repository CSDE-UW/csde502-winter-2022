...
It took 23.3 s to run. This low performance is because the algorithm is visiting every cell and comparing one-by-on with a rotating value for the correct answer from the vector of correct answers. Each object is required to be handled separately in RAM as the process continues.
...
The `adply()` version takes far less coding, but still took 23.3 s to run.
...
The `pmap()` method took 15.272 seconds to run, not much better that the other methods.
...
<hr>
Rendered at <tt>2022-03-11 18:05:05</tt>
