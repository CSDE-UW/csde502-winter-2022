...
It took 22.4 s to run. This low performance is because the algorithm is visiting every cell and comparing one-by-on with a rotating value for the correct answer from the vector of correct answers. Each object is required to be handled separately in RAM as the process continues.
...
The `adply()` version takes far less coding, but still took 21.7 s to run.
...
<hr>
Rendered at <tt>2022-02-17 22:39:15</tt>
