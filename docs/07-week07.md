...
Then we will read them back in and investigate their structure. Here we are using a temporary folder, specified using the `tmpdir()` function. Each time an R session is started, a unique temporary folder is used. For this session the temporary dir is C:\Temp\7\RtmpEBKGau. First, read the CSV format:
...
<hr>
Rendered at <tt>2022-01-08 18:00:12</tt>
