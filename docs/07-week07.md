...
First, download the [template file](files/template.txt) and save it in your course working folder as `week_07.Rmd`. Make changes to the title and author as you see fit.
...
Then we will read them back in and investigate their structure. Here we are using a temporary folder, specified using the `tmpdir()` function. Each time an R session is started, a unique temporary folder is used. For the session used to create this document, the temporary dir is C:\Temp\13\RtmpwXKXRI. 
...
<hr>
Rendered at <tt>2022-02-18 16:13:08</tt>
