...
### Downloading IDB data and creating population pyramids
In this example, we will create a population pyramid for China and India for years 2000 and 2021, following the tutorial [`idbr` README file](https://cran.r-project.org/web/packages/idbr/readme/README.html). The basic `get_idb()` function without `variables` and `concept` arguments downloads population counts.
...
#       onClick = JS("
#                    function(btn, map) {
#                       alert(map.getBounds().getEast() + (map.getBounds().getEast() - map.getBounds().getWest())/2;
...
For more things you can do with `mapview` see [mapview](https://r-spatial.github.io/mapview), particulary look at the "Articles" link at the top of the page.
